...
public class TestCaseChecker {
			...
			if (!Main.existsTestFile(getClassName(targetMethod), Main.toDirectoryName(targetMethod.getClassName()))
				...
				for (MethodInfo evMethod : evolvedMethods) {
					...
					if (r != 0) {
						...
						if (Main.testFailed(targetClassName, Main.toPackageName(targetAbsClassName))) {
	...
	public static void taihi(File dstsrcDir,File dstclassDir,String targetJavaFileName, List<String> targetClassFileNames){
	...
	public static void untaihi(File dstsrcDir,File dstclassDir,String targetJavaFileName,List<String> targetClassFileNames){
		File original = new File(dstsrcDir.getAbsolutePath()+"\\"+targetJavaFileName);
		File taihi = new File(dstsrcDir.getAbsolutePath()+"\\"+targetJavaFileName+".taihi");
